        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                if m.bias is not None:
                    m.bias.data.zero_()
                    #这段代码的主要目的是确保模型中的所有卷积层和反卷积层在训练开始时具有零偏置，这有助于模型的稳定性和训练过程的有效性。
...
class Dblock(nn.Module):
    def __init__(self,channel):
        ...
        #参数用于保持输出的空间尺寸不变。通常，padding 的值与膨胀率相同，以确保输出尺寸一致。
        #self.dilate5 = nn.Conv2d(channel, channel, kernel_size=3, dilation=16, padding=16)
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                if m.bias is not None:
    ...
    def forward(self, x):
        ...
        #dilate5_out = nonlinearity(self.dilate5(dilate4_out))
        out = x + dilate1_out + dilate2_out + dilate3_out + dilate4_out# + dilate5_out
        #这种逐层相加的方式就是一种跳跃连接的形式，目的和ResNet中的残差连接相似，都是为了更好地传递信息、保持梯度，并在多层中融合不同层次的特征。
        return out
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channels, n_filters):
        ...
        # 这是一个 1x1 的卷积层，用于减少通道数。通过降低通道数来减少计算复杂度。
        self.deconv2 = nn.ConvTranspose2d(in_channels // 4, in_channels // 4, 3, stride=2, padding=1, output_padding=1)
        self.norm2 = nn.BatchNorm2d(in_channels // 4)
        self.relu2 = nonlinearity
        #这是一个转置卷积层（也称为反卷积），用于增加特征图的空间尺寸。
        self.conv3 = nn.Conv2d(in_channels // 4, n_filters, 1)  
        self.norm3 = nn.BatchNorm2d(n_filters)
        self.relu3 = nonlinearity
        #再次使用1x1卷积，这次是将通道数从in_channels // 4降到n_filters
    def forward(self, x):
        x = self.conv1(x)
        x = self.norm1(x)
        x = self.relu1(x)
